...
Pasos:
1. Verificar que existe `data/sectores/`. Si no existe, sugerir ejecutar `/inicializar-talent-mx3` primero.
1b. Verificar si `data/sectores/` está vacío usando Glob `data/sectores/*/industria.md`.
...
2. Leer `data/indice.md` para saber qué ya existe.
...
6. Según la respuesta:
   - **Nueva industria** → asignar siguiente código SS, guiar la creación de una rama completa en `data/sectores/<nuevo-sector>/`
   - **Área funcional en sector existente** → asignar siguiente código AA dentro del sector, agregar a `data/sectores/<sector>/areas/`
...
10. Actualizar `data/indice.md` con las nuevas entradas.
